...
- По умолчанию для учётных записей **требуется 2FA (TOTP)**; **включать/отключать** это требование может только **администратор** (список пользователей). Пользователь лишь **привязывает** приложение-аутентификатор по QR-коду, если политика включена.
...
- **Пользователи**: создание, удаление (кроме системного администратора), смена роли, сброс пароля, **включение/отключение требования 2FA** (по умолчанию при создании — включено)
...
- **по коду 2FA**: на экране входа **«Забыли пароль?»** → логин/email + код 2FA + новый пароль (только если для пользователя **требуется** 2FA и секрет TOTP уже сохранён)
...
**Почта не работает, пароль забыт, войти некому** — сброс пароля с сервера по SSH: скрипт **`admin-reset-user-password.sh`** (раздел **«Скрипты»** ниже). Вторым аргументом можно **отключить требование 2FA** для этой учётной записи (`false`), если без кода TOTP войти нельзя.
...
# явно оставить требование 2FA (то же, что без второго аргумента — по умолчанию)
./scripts/admin-reset-user-password.sh user@example.com true
# отключить 2FA для пользователя (totpEnabled=false, секреты TOTP сброшены, доверенные устройства удалены)
./scripts/admin-reset-user-password.sh user@example.com false
```

- Запуск от **администратора ОС** на сервере (SSH). Первый аргумент — **email** учётной записи из таблицы `User`.
- Второй аргумент (**необязательный**) — включить или выключить **требование 2FA** для этого пользователя (`totpEnabled`): **`true`** или **`false`**. По умолчанию **`true`** (как при создании пользователя в админке). Допустимы также `1`/`0`, `yes`/`no`, `да`/`нет` (без учёта регистра).
- При **`false`**: в БД выставляются **`totpEnabled = false`**, **`totpSecret`** и **`totpTempSecret`** обнуляются, удаляются строки **`TrustedDevice`** для пользователя (как при смене политики в **Админ → Пользователи**). При **`true`**: **`totpEnabled = true`**, существующая привязка TOTP **не** сбрасывается.
...
3. **На удалённом сервере** по SSH:
   ...
   **Если после переноса нельзя войти из‑за 2FA:** на целевом сервере в PostgreSQL отключите 2FA у **всех** пользователей (подставьте пользователя/БД из env):
   ...
   Для **одной** учётной записи можно выставить новый пароль и отключить 2FA скриптом **`admin-reset-user-password.sh`** с вторым аргументом **`false`** (см. раздел **«Скрипты»** выше).

   После этого вход по паролю; требование 2FA при необходимости снова включает **администратор** в списке пользователей. Подробнее в `docs/auth.rst` (раздел «Перенос базы данных и 2FA»).
