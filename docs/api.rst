...
* **Аутентификация:** сессии (cookie). Для большинства маршрутов требуется вход; если для учётной записи **требуется** 2FA (политика администратора) — дополнительно проверка TOTP или экран первичной привязки (см. :doc:`auth`).
...
* **GET /api/auth/me** — текущий пользователь (в т.ч. ``totpEnabled``, ``totpConfigured``), флаг ``twoFactorPassed``, текущая доска (boardId).
...
* **POST /api/auth/login** — вход (email, password; опционально ``totp`` при втором шаге). Если 2FA для пользователя **не требуется**, код не нужен.
...
* **POST /api/auth/2fa/setup** — начало **привязки** TOTP (возврат секрета/QR); доступно только если для пользователя ``totpEnabled``.
* **POST /api/auth/2fa/enable** — подтверждение привязки (код из приложения, сохранение секрета).
* **POST /api/auth/2fa/verify** — проверка кода TOTP для текущей сессии (после входа).

Администрирование (только ADMIN)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

* **GET /api/auth/users** — список пользователей.
* **POST /api/auth/users** — создание пользователя (по умолчанию ``totpEnabled: true``, можно передать ``false``).
* **PATCH /api/auth/users/:id** — обновление пользователя (email, роль, **totpEnabled**; смена ``totpEnabled`` обнуляет секреты TOTP и сессии пользователя).
...
Все маршруты ниже требуют входа. Если для учётной записи **требуется** 2FA и секрет уже сохранён — в сессии должна быть отметка прохождения 2FA; если требование включено, но привязка ещё не завершена, ответ будет **403** (настройка 2FA). Если 2FA **не требуется**, достаточно входа по паролю. Где указано «контекст доски» — в сессии должен быть выбран ``boardId`` (через POST /api/boards/select).
